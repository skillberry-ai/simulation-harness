...
from simulation_harness.mcp_integration.transport import mount_sse_transport, mount_streamable_http_transport
...
def test_mount_streamable_http_transport_creates_endpoint(fastapi_app, mock_mcp_wrapper):
...
def test_transport_choice_from_config(fastapi_app, mock_mcp_wrapper):
    ...
    # Test SSE config
    with patch("simulation_harness.mcp_integration.transport.mount_sse_transport") as mock_sse:
    ...
    with patch("simulation_harness.mcp_integration.transport.mount_streamable_http_transport") as mock_http:
        config_http = Mock()
