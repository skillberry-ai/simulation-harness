...
class TestSanitizeSkillName:
    ...
    def test_spaces_become_single_hyphen(self):
    ...
    def test_caps_at_64_chars_without_trailing_hyphen(self):
        result = sanitize_skill_name("a" * 70)
        assert len(result) == 64
        assert not result.endswith("-")

    def test_cap_does_not_leave_trailing_hyphen(self):
...
class TestForceSkillName:
    def test_inserts_frontmatter_when_missing(self):
