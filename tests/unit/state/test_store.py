...
class TestList:
    ...
    def test_list_with_case_insensitive_filter(self, restaurant_store):
        ...
        results = restaurant_store.list(
            "restaurants",
            where={"location.city": {"$ieq": "boston"}}
        )
        assert len(results) >= 1
    ...
    def test_list_with_multiple_filters(self, restaurant_store):
        ...
        assert all(
            r["location"]["city"] == "Boston" and r["price_tier"] <= 3
            for r in results
        )
...
class TestUpdate:
    ...
    def test_update_existing_entity(self, restaurant_store):
        ...
        result = restaurant_store.update(
            "restaurants",
            "rest_001",
            {"rating": 4.8}
        )
...
class TestReset:
    ...
    def test_reset_restores_seed(self, restaurant_store):
        ...
        # Make some changes
        restaurant_store.insert("restaurants", {
            "id": "rest_temp",
            "name": "Temporary",
            "cuisine": "Test",
            "price_tier": 1,
...
class TestBadQueries:
    ...
    def test_unknown_operator(self, restaurant_store):
        ...
        with pytest.raises(BadQueryError):
            restaurant_store.list(
                "restaurants",
                where={"name": {"$unknown": "value"}}
            )
