...
class TestMCPToolExecution:
    ...
    def test_tools_call_executes_successfully(self, app_client, valid_openapi_spec):
        ...
        with patch('simulation_harness.skills.generator.SkillGenerator.generate_skill') as mock_gen:
            ...
            with patch('simulation_harness.agent.deep_agent.DeepAgent.generate_response') as mock_response:
                mock_response.return_value = {"result": "success", "data": [{"id": 1, "name": "Test User"}]}
    ...
    def test_tool_invocation_errors_preserve_state(self, app_client, valid_openapi_spec):
...
class TestLogging:
    ...
    def test_per_tool_call_log_line_emitted(self, app_client, valid_openapi_spec):
        ...
        with patch('simulation_harness.skills.generator.SkillGenerator.generate_skill') as mock_gen:
            ...
            with patch('simulation_harness.core.simulation_instance.logger') as mock_logger:
                # Create simulation
                response = app_client.post(
                    "/api/v1/simulation",
                    json={
