...
class SimulationStore:
    ...
    def count(self, store: str, where: dict | None = None) -> int:
        ...
        return sum(1 for e in self._data[store].values() if self._matches_where(e, where))
        ...
        for field_path, predicate in where.items():
            ...
            if isinstance(predicate, dict):
                ...
                if "$ieq" in predicate:
                    ...
                    if not isinstance(value, str) or not isinstance(predicate["$ieq"], str):
                        return False
                    ...
                    raise BadQueryError(f"Unknown operator in where clause: {list(predicate.keys())}")
